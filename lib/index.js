...
function generateXML (data){
    ...
    channel.push({ link:            data.siteUrl || 'http://github.com/dylang/node-rss' });
    // imageUrl set?
    if (data.imageUrl) {
        channel.push({ image:  [ {url: data.imageUrl}, {title: data.title},  {link: data.siteUrl} ] });
    }
    channel.push({ generator:       data.generator });
    channel.push({ lastBuildDate:   new Date().toUTCString() });

    ifTruePush(data.feedUrl, channel, { 'atom:link': { _attr: { href: data.feedUrl, rel: 'self', type: 'application/rss+xml' } } });
    ...
    ifTruePushArray(data.customElements, channel, data.customElements);
    ...
    data.items.forEach(function(item) {
        ...
        ifTruePushArray(item.customElements, item_values, item.customElements);
    ...
    Object.keys(data.customNamespaces).forEach(function(name) {
...
function RSS (options, items) {
    ...
    this.siteUrl           = options.siteUrl || options.site_url;
    this.imageUrl          = options.imageUrl || options.image_url;
    ...
    this.customNamespaces  = options.customNamespaces || options.custom_namespaces ||{};
    this.customElements    = options.customElements || options.custom_elements || [];
    this.items              = items || [];
    ...
    this.item = function (options) {
        ...
        var item = {
            ...
            customElements:  options.customElements || options.custom_elements || []
        };
