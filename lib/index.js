...
function generateXML (data){
    ...
    data.items.forEach(function(item) {
        ...
        if( item.enclosure && item.enclosure.url) {
            ...
            var enclosure_attributes = { url: item.enclosure.url };
            if( item.enclosure.file ) {
                enclosure_attributes.length = item.enclosure.size || getSize(item.enclosure.file);
                enclosure_attributes.type = mime.lookup(item.enclosure.file);
            } else {
                enclosure_attributes.length = item.enclosure.size || 0;
                enclosure_attributes.type  = mime.lookup(item.enclosure.url);
            }

            if (item.enclosure.custom_attributes){
                Object.keys(item.enclosure.custom_attributes).forEach(function(name){
                    enclosure_attributes[name] = item.enclosure.custom_attributes[name];
                });
            }

            item_values.push({
                enclosure: {
                    _attr: enclosure_attributes
                }
            });
        }
